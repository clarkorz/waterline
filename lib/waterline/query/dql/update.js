...
function createBelongsTo(valuesObject, cb) {
  ...
  async.each(valuesObject.associations.models, function(item, next) {
    ...
    query.exec(function(err, val) {
      ...
      if(!val) return next(new WLValidationError({
        invalidAttributes: _.zipObject([item], [[{ message: 'Associated model does not exist.'}]]),
        model: self._model.globalId
      }));
...
function updatedNestedAssociations(valuesObject, values, cb) {
  ...
  associations.models = _.filter(associations.models, function(model) {
    ...
    var attribute = self._schema.schema[model];
    return !attribute.embed && (_.isPlainObject(vals) || Array.isArray(vals) ? true : false);
  });

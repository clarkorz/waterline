...
module.exports = function() {
  ...
  Object.keys(this.connections).forEach(function(conn) {
    ...
    Object.keys(adapter).forEach(function(key) {
      ...
      // the first argument
      self[key] = function(/* [criteria], [...restArgs], [cb] */) {
        "use strict";
        var criteria, restArgs;

        if (arguments.length == 1 && _.isFunction(arguments[0])) {
          criteria = undefined;
          restArgs = Array.prototype.slice.call(arguments);
        } else {
          criteria  = _.first(arguments);
          restArgs  = _.rest(arguments);
        }
        ...
        function adapterMethod(criteria, cb) {
          ...
          var args = [conn, tableName];

          if (criteria) {
            // Normalize criteria and fold in options
            criteria = normalize.criteria(criteria);
            args = args.concat(criteria);
          }
